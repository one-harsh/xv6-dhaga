...
void acq_proc(struct proc *p) {
  acquire(&p->lock);
}

void rel_proc(struct proc *p) {
  release(&p->lock);
}

void acq_thread(struct thread *t) {
  acquire(&t->lock);
}

void rel_thread(struct thread *t) {
  release(&t->lock);
}
...
int createThread(uint64 va) {
  ...
  acq_proc(t->parentProc);
  if (!t->parentProc->threads[1]) {
    uint64 oldsz = t->parentProc->sz;
    // This proc has only had the main thread until this point, so allocate space for thread stacks
    if (growproc((NTHREADPERPROC - 1) * THREADSTACKSIZE) < 0) {
      rel_proc(t->parentProc);
      return 0;
    }

    for (i = 1; i < NTHREADPERPROC; i++) {
      t->parentProc->stacks[i] = oldsz + THREADSTACKSIZE * i;
    }
  }
  ...
  memset(t->tf, 0, sizeof t->tf); // BUGBUG Why clearing trapframe of current thread?
...
exit(int status)
{
  ...
  // harmless.
  acq_thread(initproc->threads[0]);
  wakeup1(initproc->threads[0]);
  rel_thread(initproc->threads[0]);
  ...
  acq_thread(original_parent->threads[0]);
  ...
  rel_thread(original_parent->threads[0]); // should complain
